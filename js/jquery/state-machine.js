(function (window) {
  ...
  StateMachine = {
    ...
    create: function(cfg, target, view) {
    	
    	
      //view.bind('fsm:' + 'name', this.activate_buttons);
    ...
    buildEvent: function(name, map) {
      return function() {
        ...
        if (self.transition)
          return self.error(name, from, to, args, StateMachine.Error.PENDING_TRANSITION, "event " + name + " inappropriate because previous transition did not complete");

        if (self.cannot(name))
          return self.error(name, from, to, args, StateMachine.Error.INVALID_TRANSITION, "event " + name + " inappropriate in current state " + self.current);
        ...
        if (false === leave) {
          ...
          if (self.transition)
            self.transition(); // in case user manually called transition() but forgot to return ASYNC
