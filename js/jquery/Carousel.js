...
(function($) {
	$.fn.jCarouselLite = function(o) {
		o = $.extend({
			...
			start: 0,
			scroll: 1,
			rows: 2, //ADDED: Property to allow multiple rows
			beforeStart: null,
			afterEnd: null
		}, o || {});
		...
		return this.each( function() { //Returns the element collection. Chainable.
			...
			//ADDED: special consideration for vertical carousels withultiple rows
			...
			function go(to) {
				if (!running) {
					...
					if (o.circular) {            // If circular we arein first or last, then goto the other end
						if (to <= o.start - v - 1) {           // Iffirst, then goto last
							ul.css(animCss, -((itemLength - (v * 2)) * liSize) + "px");
							// If "scroll" > 1, then the "to" mightnot be equal to the condition; it can be lesser depending on the number of elements.
							curr = to == o.start - v - 1 ? itemLength
							- (v * 2) - 1 : itemLength - (v * 2) - o.scroll;
						} else if (to >= itemLength - v + 1) { // Iflast, then goto first
